        ...
        <div className="container mx-auto px-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
                {skills.map((skill) => (
                    <article
                        key={skill.category}
                        className="bg-gray-50 p-8 rounded-2xl border border-gray-200 hover:border-blue-200 transition-colors"
                    >
                        <div className="flex justify-center mb-6">
                            <div className="p-4 bg-white rounded-full shadow-sm">
                                {React.cloneElement(skill.icon, { className: 'h-8 w-8 text-blue-600' })}
                            </div>
                        </div>
